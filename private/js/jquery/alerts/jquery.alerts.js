...
(function($) {
		...
		_show: function(title, msg, value, type, size, id, callback) {
			...
			switch( type ) {
				case 'loading':
					...
					Loading_Overly.load();
					$(".LightningUpdating").css('z-index',20000);
					$(".LightningUpdating").next().css('z-index',19999);
					$(':focus').blur();
					break;
				case 'alert':
					...
					$(".LightningStatusPanel").css('z-index',20000);
					$(".LightningStatusPanel").next().css('z-index',19999);
					...
					switch(title)
					{
						...
						case 'loading':
							...
							break;
						case 'note':	//Cloud and Setting/Media GUI: iTune On will show info msg.
							_TITLE= _T('_cloud','msg7');
							icon_obj.addClass('LightningStatusPanelIconInfo');
							break;	
						case 'usb_unlock_exceeded_title':
							_TITLE= _T('_home','usb_unlock_exceeded_title');
							break;
						case 'not_allowed_title':
							_TITLE= _T('_cloud','not_allow');
							break;
						case 'add_an_app':
							_TITLE = _T("_module","desc27");
							icon_obj.addClass('LightningStatusPanelIconComplete');
						break;
						case 'privacy':
							_TITLE = _T("_support","privacy");
							break;
						default:
							if(title.length == 0) title = _T('_common','completed');
							_TITLE = title;
							icon_obj.addClass('LightningStatusPanelIconComplete');
							break;
					}
				...
				case 'confirm':
					...
					var alertObj = $('.LightningStatusPanel').overlay({fixed:false,oneInstance:false,expose: '#333333',api:true,closeOnClick:false,closeOnEsc:false});
					...
					switch(id)
					{
						...
						case 'ssh':
						case 'ssh2':
							//$(".PanelButton").css("left","95px");
							$(".LightningStatusPanelBody").css("min-width","500px");
							$(".LightningStatusPanel").css("min-width","500px");
					...
					{
						case 'cloud_signin':
							$(".LightningStatusPanelBody").css('width','530');
							$("body.RU-RU .LightningStatusPanelBody").css('width','680');
							$("body.ES-ES .LightningStatusPanelBody").css('width','580');
							$("#popup_apply_button").text(_T('_migration','yes_attach_to_same_email'));
							$("#popup_close_button").text(_T('_migration','no_use_different_email'));
							break;
						case 'cloud2':
							$("body.HU-HU .LightningStatusPanelBody").css('width','auto');
							break;
						case 'ipv4':
							$("#popup_apply_button").text(_T('_button','save'));
							$("#jConfirm_apply").css('left','10px');
							break;
						case 'ipv6':
							$("#popup_apply_button").text(_T('_button','continue'));
							$("#jConfirm_apply").css('left','10px');
							break;
						case 'r_share':
							setTimeout("$('#create_folder_tb input[name=backups_rNewFolder_text]').focus()",100);
							init_select();
							break;
						case 'ssh':
							if(_SSH_PW_STATUS!=0)
							{
								switch(_SSH_PW_STATUS)
								{
									case -1:
										...
										setTimeout("$('#ssh_pw_tb input[name=settings_SSHPW_password]').focus()",100);
										break;
									case -2:
										show_error_tip(".tip_pw_error",_T('_pwd','msg12'));
										setTimeout("$('#ssh_pw_tb input[name=settings_SSHPW_password]').focus()",100);
										break;
									case -3:
									...
									case -6:
										...
										setTimeout("$('#ssh_pw_tb input[name=settings_SSHConfirmPW_password]').focus()",100);
										break;
								}
							...
							break;
						case 'fw':
							$("#popup_apply_button").text(_T('_button','install'));
							break;	
					}
	...
	jLoadingClose = function(callback) {
		if(Loading_Overly)Loading_Overly.close();
