...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		fileTree: function(o, callback) {
			...
			// for scroll bar
			if (o.share == undefined) o.share = "";//for show share path

			/* [+] multi select, max select, error message */
			...
			/* [-] multi select, max select, error message */
			if (o.checkbox_click_callback == undefined) o.checkbox_click_callback = null; //for checkbox click event
			...
			switch(o.ID)
			{
				...
				case 'img_tree_div2':
					...
					break;
				case 'rsync_tree_div':
					o.scrollID = '.remoteBackup_scroll_pane';
					if(!$(this).parent().hasClass('jspPane'))
					{
						$(this).wrap('<div class="remoteBackup_scroll_pane">');
					}
					break;
				default:
				...
				function showTree(c, t) {
					...
					if ( $(c).hasClass("directory") && $(c).hasClass("collapsed") && $(c).hasClass("add"))
					{
						...
						jPrompt(_T('_network_access', 'input_folder_name'), "", _T('_network_access', 'create_folder_name'), function(r) {
							if (r)
							{
								var ret = $("#new_folder_name").val();

								//for safari ,multi-line issue
								var s = "";
								...
								var filename = unescape(ret).trim();
									$.post(o.script, {
										dir: unescape(new_f),
										filename:filename,
					...
					$.post(o.script, { dir: unescape(t), cmd: o.cmd, show_file: __file ,chk_flag:__chkflag,file_type:o.filetype,function_id:o.function_id,filter_file:o.filter,root_path:o.root_path, host: o.host, user:o.user, pwd:o.pwd, lang:o.lang}, function(data) {
						var new_folder = "";
						...
						$(data).find('LI').each(function(i){
							if( $(this).hasClass('directory') )
							{
								...
								if (o.checkbox_all == 2)//show checkbox,support file and folder to selected
								{
									...
									{
										v = htmlencode($(this).find('a').attr("rel"));
										if (o.new_folder == t)
											new_text += '<a href="#" rel="' + v + '/" id="get_focus">' + t +'</a>';
										else
											new_text += '<a href="#" rel="' + v + '/">' + t +'</a>';
								...
								{
									let v = htmlencode($(this).find('a').attr("rel"));
									if (o.new_folder == t)
										new_text += '<a href="#" rel="' + v + '/" id="get_focus">' + t +'</a>';
									else
										new_text += '<a href="#" rel="' + v + '/">' + t +'</a>';
						...
						$("input[name=folder_name]").click(function(){
							var sel_source_ele = $("input:checkbox:checked[name=folder_name]:not([disabled])");

							if ($(this).prop("checked")) //not select -> select
							{
								...
								if (o.single_select) //Single select
								{
								...
								else //Multi select
								{
									...
									p.find("ul input").prop("checked", true).prop('disabled', true);
									p.find("ul .LightningCheckbox").removeClass('gray_out').addClass('gray_out');
								}
							}
							else
							{
								$(this).prop("checked", false);
								var p = $(this).parent().parent();
								p.find("ul input").prop("checked", false).prop('disabled', false);
								p.find("ul .LightningCheckbox").removeClass('gray_out');
								p = p.parent().parent();

								while (p.find('input').length > 0)
								{
									if (p.parent().parent().find(' > li input').length == 0) //root
										break;

									p.find('input').eq(0).prop("checked", false);
									p = p.parent().parent();
								}
							}

							if (o.checkbox_click_callback && o.checkbox_all == 2)
								o.checkbox_click_callback(o);
						});
						/* [-] multi select, max select, error message */
						...
						$("#iso_tree_container input[name=folder_name]").click(function(){
					 		...
					 		$("#iso_tree_container input:checkbox").prop("checked",false);
							$(this).prop("checked", true);

							if($(this).prop("checked") == true)
							{
								var status = iso_tree_check_share($(this).val());
						...
						/* auto focus and scroll to new folder */
						...
						if (o.multi_select && !o.single_select) //Multi select
						{
							if ($(c).find('input').prop("checked")) //Select
							{
								$(c).find('ul input').prop("checked", true).prop('disabled', true);
								$(c).find("ul .LightningCheckbox").removeClass('gray_out').addClass('gray_out');
							}
						}
				...
				function chg_path(t,form_obj,text_obj)
				{
					...
					if ( ( t != "/mnt/HD") && ( t !="/mnt/USB") )
					{
						...
						if (o.function_id == "iso_create")
						{
							...
							var show_text = t.substr(len,t.length-len+1);
							if (text_obj != null  && show_text != "")
								document.getElementById(text_obj).value=unescape(show_text);
						...
						if( $(this).parent().hasClass('directory') ) {
							...
							switch(o.function_id)
							{
								case "iso_create":
									...
									full_path = full_path.substr(0, full_path.lastIndexOf("/")+1);
