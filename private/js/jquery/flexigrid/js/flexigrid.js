...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			cmd:false,
			id:false, //amy add
			pclass:'class',//amy add
			change:false, //amy add
			f_field:false,  //+alpah.eve on 2009/10/27
			user:'' //fish add for my musics
			/*[-] ALPHA CUSTOMIZE */
		}, p);
		...
		var g = {
			...
			populate: function () { //get latest data
				/* [+] ALPHA Customize, alpha.amy mark */
				//if (this.loading) {
				//	return true;
				//}
				/* [-] ALPHA Customize */
					...
					wd_ajax({	
					   ...
					   success: function(xml){
					   		...
					   		function _get_file()
					   		{
								...
								var _url = "/xml/mymusic-editlist-" + login_name + "-" + p.f_field + ".xml";
								wd_ajax({
									type: p.method,
									url: _url,
									dataType: p.dataType,
...
//				   	if(p.height=='auto')
//				   	{
//				   		var FIG_Height = ($(g.bDiv).height()+ 4) + "px";
//							$(g.bDiv).css('height', '100%');								
//				   	} 
				   	 
				   	//alpha.amy++				 
//				   	if (p.id =="flex_session_new_account")
//				   	{						   		
//				   		re_add_user_list();
//				   	}	
				 	if (p.pclass == "account")
				   	{
				   		//revise_user_table_len();
				   		//revise_group_table_len();
				   	}
				   	else if (p.pclass == "log")
				   	{					   		
				   		//revise_log_table_len();
					  }
						
				
				   	if ( p.pclass =="flex_account1" )
				   	{
				   		$("input[name=r_chkAll]").attr('checked',false);
						$("input[name=w_chkAll]").attr('checked',false);	
								   		
				   		if (p.cmd == "cgi_get_ftp_group_list")
				   		{	
				   			ftp_add_group_list();
				   			re_add_ftp_group_list(p.f_field);
				   		}

				   		if (p.cmd == "cgi_get_ftp_user_list")
				   		{
				   			ftp_add_user_list();
				   			re_add_ftp_user_list(p.f_field);
				   		}
				   	}
				   	
				   	if ( p.pclass =="flex_account2" )
				   	{
				   		$("input[name=m_r_chkAll]").attr('checked',false);
						$("input[name=m_w_chkAll]").attr('checked',false);
							   		
				   		if (p.cmd == "cgi_get_ftp_group_list")
				   		{	
				   			m_ftp_add_group_list();
				   			re_add_ftp_group_list(p.f_field);
				   		}
				    ...
				    if (p.pclass == "flex_account")
				   	{					   						   						   			
				   		$("input[name=cifs_r_chkAll]").attr('checked',false);
						$("input[name=cifs_w_chkAll]").attr('checked',false);	
						$("input[name=cifs_d_chkAll]").attr('checked',false);	
				   				   						   						   			
				   		if (p.cmd.indexOf("group") == -1 )
				   		{				   		
					 		cifs_add_user();	
					   		cifs_readd_user();						   		
					   	}
...
//				   	else if (p.cmd == "cgi_get_session")
//			   		{			   			
//			   			setTimeout(load_tooltip,1000);			   			
//			   		}
//			   		else if (p.cmd == "cgi_get_ftp_list")
//			   			setTimeout(load_ftp_tooltip,1000);
//				   	//alpha.amy--	
			...
			changePage: function (ctype) { //change page
				...
				p.change = false; //amy add, ALPHA Customize
				if (p.newp == p.page) {
					return false;
				}
				if (p.onChangePage) {
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					//[+] amy++, ALPHA CUSTOMIZE
                    var button_id = "flexigrid_" + btn.bclass;
                    //[-] amy--, ALPHA CUSTOMIZE
					...
					//[+] amy++, ALPHA CUSTOMIZE
						if (btn.disabled)	
						{
							...
							//amy--
						btnDiv.bid = button_id; //amy07212010++
					//[+] amy--, ALPHA CUSTOMIZE
					...
					if (btn.onpress) {
						$(btnDiv).click(function () {
							if (!$("#"+this.bid).hasClass('gray')) //amy07212010++ for disabled item, ALPHA CUSTOMIZE
...
/* [+] ALPHA_CUSTOMIZE */
	//Add by Ben, 2013/10/01
	$.fn.flexAjaxID = function(p) { // function to get ajaxid
...
//amy++
	$.fn.flexField = function(p) { // function to get cmd
	...
	$.fn.flexCmd = function(p) { // function to get cmd
	...
	$.fn.flexRp = function(p) { // function to get cmd
	...
	$.fn.flexPage = function(p) { // function to get cmd
...
//amy--
//fish++
	$.fn.flexTotal = function(p) { // function to get total item
